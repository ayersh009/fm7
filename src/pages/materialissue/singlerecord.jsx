...
const SingleRecordPage = ({ f7route, f7router }) => {
  ...
  const shareRecord = async () => {
    const detailsText = details
      .map(
        detail =>
          `Item: ${detail.product}, Quantity: ${detail.qty}, Unit: ${detail.uom}`
      )
      .join('\n')
    ...
    <Page>
      <Navbar title='Record Details' backLink='Back'>
        <Link
          ...
          onClick={() =>
            f7router.navigate(
              `/edit-record/${encodeURIComponent(JSON.stringify(item))}`
            )
          }
        />
        <Link
      ...
      <List>
        <ListItem onClick={() => standalone.current.open()}>
          <img src={item.mrn_photo} alt='MRN Photo' style={{ width: '90%' }} />
        </ListItem>
