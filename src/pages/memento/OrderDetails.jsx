import React, { useState, useEffect } from 'react'
import {
  Card,
  CardContent,
  Button,
  f7,
  Popup,
  Page,
  List,
  ListInput,
  Navbar,
  Block,
  Icon,
  CardHeader,
  Link
} from 'framework7-react'
import { supabase } from '../../components/supabase'

const DetailsDataTable = ({ orderid }) => {
    const [details, setDetails] = useState([])
    const [isPopupOpen, setIsPopupOpen] = useState(false)
    const [editForm, setEditForm] = useState({})
    const [isEdit, setIsEdit] = useState(false)
  
    const fetchDetails = async () => {
      try {
        const { data, error } = await supabase
          .from('orderdetails')
          .select('*')
          .eq('orderid', orderid)
  
        if (error) {
          console.error('Error fetching details:', error.message)
          f7.dialog.alert('Error fetching details. Please try again.', 'Error')
        } else {
          setDetails(data)
        }
      } catch (error) {
        console.error('Error fetching details:', error.message)
        f7.dialog.alert('Error fetching details. Please try again.', 'Error')
      }
    }
  
    useEffect(() => {
      fetchDetails() // Fetch details when component mounts or when `orderid` changes
    }, [orderid])
  
    const handleDeleteClick = async id => {
      try {
        const { error } = await supabase
          .from('orderdetails')
          .delete()
          .eq('id', id)
  
        if (error) {
          console.error('Error deleting record:', error.message)
          f7.dialog.alert('Error deleting record. Please try again.', 'Error')
        } else {
          setDetails(details.filter(detail => detail.id !== id))
          f7.dialog.alert('Record deleted successfully.', 'Success')
        }
      } catch (error) {
        console.error('Error deleting record:', error.message)
        f7.dialog.alert('Error deleting record. Please try again.', 'Error')
      }
    }
  
    const handleEditClick = detail => {
      setEditForm(detail)
      setIsEdit(true)
      setIsPopupOpen(true)
    }
  
    const handleAddClick = () => {
      setEditForm({ orderid })
      setIsEdit(false)
      setIsPopupOpen(true)
    }
  
    const handleInputChange = e => {
      const { name, value } = e.target
      setEditForm(prev => ({ ...prev, [name]: value }))
    }
  
    const handleSaveClick = async () => {
      try {
        if (isEdit) {
          const { error } = await supabase
            .from('orderdetails')
            .update(editForm)
            .eq('id', editForm.id)
  
          if (error) {
            console.error('Error updating record:', error.message)
            f7.dialog.alert('Error updating record. Please try again.', 'Error')
          } else {
            const newDetails = [...details]
            const index = newDetails.findIndex(
              detail => detail.id === editForm.id
            )
            newDetails[index] = editForm
            setDetails(newDetails)
            setIsPopupOpen(false)
            f7.dialog.alert('Record updated successfully.', 'Success')
          }
        } else {
          const { data, error } = await supabase
            .from('orderdetails')
            .insert([editForm])
            .select() // Explicitly select the inserted row
  
          if (error) {
            console.error('Error adding record:', error.message)
            f7.dialog.alert('Error adding record. Please try again.', 'Error')
          } else if (data && data.length > 0) {
            setDetails([...details, data[0]])
            setIsPopupOpen(false)
            f7.dialog.alert('Record added successfully.', 'Success')
          } else {
            console.error('Error adding record: No data returned from Supabase')
            f7.dialog.alert('Error adding record. Please try again.', 'Error')
          }
        }
      } catch (error) {
        console.error('Error saving record:', error.message)
        f7.dialog.alert('Error saving record. Please try again.', 'Error')
      }
    }

  return (
    <div>
      <Card className='data-table data-table-init'>
        <CardHeader>
          <div className='data-table-links'>
            <a className='button' onClick={handleAddClick}>
              <Icon f7='plus-filled' size='24px' color='blue' padding></Icon>{' '}
              Add Product
            </a>
          </div>
        </CardHeader>
        <CardContent padding={false}>
          <table>
            <thead>
              <tr>
                <th className='label-cell'>PRODUCT</th>
                <th className='label-cell'>QUANTITY</th>
                <th className='actions-cell'>ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {details.map(detail => (
                <tr key={detail.id}>
                  <td className='label-cell'>{detail.product}</td>
                  <td className='label-cell'>{detail.quantity}</td>
                  <td className='actions-cell'>
                    <Link
                      iconIos='f7:square_pencil'
                      iconMd='material:edit'
                      onClick={() => handleEditClick(detail)}
                    />
                    <Link
                      iconIos='f7:trash'
                      iconMd='material:delete'
                      onClick={() => handleDeleteClick(detail.id)}
                    />
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </CardContent>
      </Card>

      <Popup opened={isPopupOpen} onPopupClosed={() => setIsPopupOpen(false)}>
        <Page>
          <Navbar
            title={isEdit ? 'Edit Record' : 'Add Record'}
            backLink='Back'
          />
          <List noHairlinesMd>
            <ListInput
              label='Product'
              type='text'
              placeholder='Enter Product'
              name='product'
              value={editForm.product || ''}
              onInput={handleInputChange}
            />
            <ListInput
              label='Quantity'
              type='text'
              placeholder='Enter Quantity'
              name='quantity'
              value={editForm.quantity || ''}
              onInput={handleInputChange}
            />
          </List>
          <Block>
            <Button fill onClick={handleSaveClick}>
              {isEdit ? 'Save Changes' : 'Add Record'}
            </Button>
          </Block>
        </Page>
      </Popup>
    </div>
  )
}

export default DetailsDataTable
