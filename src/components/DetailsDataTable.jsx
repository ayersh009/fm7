...
import { Card, CardContent, Button, f7, Popup, Page, List, ListInput, Navbar, Block } from 'framework7-react';
...
const DetailsDataTable = ({ mrn_no }) => {
    ...
    try {
      if (isEdit) {
        ...
        if (error) {
          ...
          const index = newDetails.findIndex((detail) => detail.id === editForm.id);
  ...
  return (
    <div>
              ...
              {details.map((detail) => (
                <tr key={detail.id}>
                  ...
                  <td className="actions-cell">
                    ...
                    <Button onClick={() => handleDeleteClick(detail.id)}>Delete</Button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Block>
        <Button fill onClick={handleAddClick}>Add Record</Button>
      </Block>

      <Popup opened={isPopupOpen} onPopupClosed={() => setIsPopupOpen(false)}>
        <Page>
          <Navbar title={isEdit ? 'Edit Record' : 'Add Record'} backLink="Back" />
          <List noHairlinesMd>
            <ListInput
              label="Product"
              type="text"
          ...
          <Block>
            <Button fill onClick={handleSaveClick}>{isEdit ? 'Save Changes' : 'Add Record'}</Button>
          </Block>
        </Page>
      </Popup>
    </div>
